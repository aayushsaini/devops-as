...
# Ensure the repository `git` folder is on sys.path so `from app import app` works
...
def test_get_env(set_env_vars):
    ...
    assert response.status_code == 200
    assert response.json() == {"app_name": "Test App","environment": "test","secret_key": "test-secret-key"}
